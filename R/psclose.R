`psclose` <-
function(file=getOption("gmt.file"), cleanup=TRUE)
{
  ...
  file.create(tmp)
  gmt.system(paste("psxy", tmp, "-JM -R -O"), file=file, append=TRUE)  # slightly larger file than -O in last cmd
  all.tmp <- paste(dirname(tempdir()), c("bar.gmt","empty.gmt","text.gmt","xy.gmt"), sep="/")
  unlink(all.tmp)

  ## 2 Move bounding box to top, so Ghostscript can distill -dEPSCrop
  ps <- readLines(file)
